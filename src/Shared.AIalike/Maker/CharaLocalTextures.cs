...
namespace KKAPI.Maker
{
    public static partial class CharaLocalTextures
    {
        ...
        private static GameObject SetupUIPanel(bool forceNew, out bool isNewPanel)
        {
            ...
            tglLocal.isOn = SaveType == CharaTextureSaveType.Local;
            tglLocal.onValueChanged = new Toggle.ToggleEvent();
            tglLocal.onValueChanged.AddListener((x) =>
            {
                if (x && !saveTypeChanging)
                    SaveType = CharaTextureSaveType.Local;
            });
            ...
            {
                CharaTextureSaveType newValue = y.NewSetting;
                if (newValue == CharaTextureSaveType.Bundled && !tglBundled.isOn && tglBundled.isActiveAndEnabled)
                    tglBundled.isOn = true;
                else if (newValue == CharaTextureSaveType.Local && !tglLocal.isOn && tglLocal.isActiveAndEnabled)
                    tglLocal.isOn = true;
            };
