...
using TransformGizmoAPI.Helpers;
using TransformGizmoAPI.Objects;
using TransformGizmoAPI.Objects.Commands;
using TransformGizmoAPI.UndoRedo;
using UnityEngine;
...
namespace TransformGizmoAPI
{
	...
	// Fork of https://github.com/HiddenMonk/Unity3DRuntimeTransformGizmo
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		void Update()
		{
			...
			//todo rename to specify it's a cursor raycast into any collider? setting to enable?
			GetTarget();
				...
				if(previousMousePosition != Vector3.zero && mousePosition != Vector3.zero)
				{
					...
					else if(transType == TransformType.Rotate)
					{
						...
						if(nearAxis == Axis.Any)
						{
							...
							{
								float angle = SignedAngle(previousMousePosition - originalPivot, mousePosition - originalPivot, axis);
		...
		void SetMaterial()
		{
			...
			{	
                ...
                lineMaterial = new Material(TransformGizmoAPI._lineShader);
                outlineMaterial = new Material(TransformGizmoAPI._outlineShader);
			}
		}
	}
}
