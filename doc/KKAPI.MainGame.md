...
| Type | Name | Summary | 
| --- | --- | --- | 
| `IDisposable` | AddActionIcon(`Int32` mapNo, `Vector3` position, `Sprite` iconOn, `Sprite` iconOff, `Action` onOpen, `Action<TriggerEnterExitEvent>` onCreated = null, `Boolean` delayed = True, `Boolean` immediate = False) | Register a new action icon in roaming mode (like the icons for training/studying, club report screen, peeping).  Dispose the return value to remove the icon.  Icon templates can be found here https://gitgoon.dev/IllusionMods/IllusionModdingAPI/tree/master/src/KKAPI/MainGame/ActionIcons | 
